...
class Model12(object):
    ...
    def __init__(self):
        ...
        with tf.variable_scope("scope1") as scope:
            net_conv = Model1.make_core_network(inputs)
        with tf.variable_scope("scope2") as scope:
            net_dnn = Model2.make_core_network(inputs)

        network = tf.concat([net_conv, net_dnn], 1, name="concat")
    ...
    def load_from_two(self, m1fn, m2fn):
        self.model.load(m1fn, scope_for_restore="scope1", weight_only=True)
        self.model.load(m2fn, scope_for_restore="scope2", weight_only=True,
...
def prepare_model1_weights_file_in_scopeQ():
    ...
    with tf.variable_scope("scopeQ") as scope:
        m1 = Model1()
    m1.model.fit({"scopeQ/input": X}, {"scopeQ/target": Y},
                 n_epoch=1, validation_set=0.1,
                 show_metric=True, run_id="model1_scopeQ")
...
def demonstrate_loading_weights_into_diffetent_scope():
...
def demonstrate_loading_weights_into_diffetent_scope_using_custom_function():
...
def demonstrate_loading_two_instances_of_model1():
    ...
    with tf.variable_scope("scopeA") as scope:
        ...
        try:
            m1a.model.load("model1.tfl", weight_only=True)
        ...
        m1a.model.load("model1.tfl", scope_for_restore="scopeA", verbose=True, weight_only=True)

    with tf.variable_scope("scopeB") as scope:
        m1b = Model1()
        m1b.model.load("model1.tfl", scope_for_restore="scopeB", verbose=True, weight_only=True)
    print("="*60 + " Successfully restored weights to two instances of model1, in different scope")
...
def demonstrate_combined_model1_and_model2_network():
    ...
    print("-"*40 + " Creating mashup of model1 and model2 networkers")
    tf.reset_default_graph()
    ml2 = Model12()
    print("-"*60 + " Loading model1 and model2 weights into mashup")
    ml2.load_from_two("model1.tfl", "model2.tfl")
    print("-"*60 + " Training mashup")
    ml2.train(X, Y, testX, testY, 1)
    print("-"*60 + " Saving mashup weights")
    ml2.model.save("model12.tfl")
    print("-"*60 + " Done")
...
demonstrate_loading_weights_into_diffetent_scope()
demonstrate_loading_weights_into_diffetent_scope_using_custom_function()
